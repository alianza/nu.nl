...
export const StoryService = {
  ...
  transformSelectors: [
    ...
    { selector: 'span.date', transform: (e) => { e.style.fontStyle = 'italic'; e.parentElement.style.textAlign = 'right' } },
    { selector: 'img[data-src*="media.nu.nl"]', transform: (e) => {e.src = e.getAttribute('data-src') } },
    { selector: 'img[src*="media.nu.nl"]', transform: (e) => { e.style.margin = "1em auto"} },
    { selector: 'a[href^="/"]', transform: (e) => {e.setAttribute('href', `https://nu.nl${e.getAttribute('href')}`)} },
  ...
  async fetchStory(url) {
    const response = await fetch(`${this.baseUrl}get?url=${encodeURIComponent(url)}`)
  ...
  transformStory(story, storyObj) {
    ...
    this.transformSelectors.forEach(selector => story.querySelectorAll(selector.selector).forEach(el => selector.transform(el)))
